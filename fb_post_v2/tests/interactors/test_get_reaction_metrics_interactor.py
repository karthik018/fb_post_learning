...
from fb_post_v2.interactors.storages.post_storage \
    import PostStorage, ReactionCountDTO
...
class TestReactionMetrics(unittest.TestCase):
    ...
    def test_reaction_metrics(self):
        ...
        reaction1_metrics_dto = Mock(
            spec=[field.name for field in fields(ReactionCountDTO)])

        reaction2_metrics_dto = Mock(
            spec=[field.name for field in fields(ReactionCountDTO)])

        reaction3_metrics_dto = Mock(
            spec=[field.name for field in fields(ReactionCountDTO)])

        post_storage_mock = Mock(spec=PostStorage)
        presenter_mock = Mock(spec=JsonPresenter)
        reaction_metrics = GetReactionMetricsInteractor(post_storage_mock,
                                                        presenter_mock)
        reactions_dto = [reaction1_metrics_dto, reaction2_metrics_dto,
                         reaction3_metrics_dto]
