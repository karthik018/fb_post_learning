...
from fb_post_v2.interactors.storages.post_storage import UserPostsDTO, \
    PostStorage
...
class TestUserPosts(unittest.TestCase):
    def test_user_posts(self):
        user_posts_dto = Mock(
            spec=[field.name for field in fields(UserPostsDTO)])
        post_storage_mock = Mock(spec=PostStorage)
        presenter_mock = Mock(spec=JsonPresenter)
        get_user_posts = GetUserPostsInteractor(post_storage_mock,
                                                presenter_mock)
