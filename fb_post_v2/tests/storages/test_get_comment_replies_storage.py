...
class TestGetReplies:
    ...
    def setup_data(self):
        ...
        self.first_reply = Comment.objects.create(post_id=self.post.id, user_id=self.user.id,
    ...
    def test_get_replies(self, setup_data):
        ...
        replies_dto = post_storage.get_comment_replies(self.first_comment.id, offset=0, limit=1)

        assert len(replies_dto) == 1

        assert replies_dto[0].comment_id in [self.first_reply.id, self.second_reply.id]
