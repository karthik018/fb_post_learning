...
class TestPostsReactedByUser:
    ...
    def setup_data(self):
        ...
        self.post2 = Post.objects.create(user_id=self.user.id, post_description="second post")
        self.first_reaction = PostReaction.objects.create(post_id=self.post.id, user_id=self.user.id, reaction="LIKE")
        self.second_reaction = PostReaction.objects.create(post_id=self.post2.id, user_id=self.user.id,
                                                           reaction="LOVE")

    def test_posts_reacted_by_user(self, setup_data):
        post_storage = PostStorage()

        posts_reacted = post_storage.get_posts_reacted_by_user(self.user.id)

        assert len(posts_reacted) == 2

        posts_ids = [post_dto.post_id for post_dto in posts_reacted]
