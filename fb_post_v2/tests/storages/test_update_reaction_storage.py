...
class TestUpdatePostReaction:
    ...
    def setup_data(self):
        ...
        self.first_Reaction = PostReaction.objects.create(user_id=self.user.id, post_id=self.first_post.id, reaction="LIKE")

    def test_delete_post_reaction(self, setup_data):
        post_storage = PostStorage()

        reaction_id_dto = post_storage.update_post_reaction(self.first_post.id, self.user.id, "LOVE")

        assert reaction_id_dto.reaction_id == self.first_Reaction.id
...
class TestUpdateCommentReaction:
    ...
    def test_delete_comment_reaction(self, setup_data):
        ...
        reaction_id_dto = post_storage.update_comment_reaction(self.first_comment.id, self.user.id, "LOVE")

        assert reaction_id_dto.reaction_id == self.first_comment_reaction.id
